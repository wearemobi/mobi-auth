...
public class JwtService {
  ...
  /**
   * Generates the Access Token (Daily Combat Key) Includes granular roles and tenant identification
   * for RBAC.
   */
  public String generateToken(MobiUser user) {
    ...
    var claims =
        Map.of(
            "roles",
                List.of(Role.valueOf(user.role()).getAuthority()), // 🚀 Mapping to ROLE_ format
  ...
  /**
   * Generates the Refresh Token (Long-term Master Key) Minimal claims to ensure security and
  ...
  /**
   * Extracts all claims from the provided Token Performs signature verification against the
   * internal SecretKey.
   */
